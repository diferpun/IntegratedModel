...
if __name__ == '__main__':
   ...
   # medir tiempo
   # implementar el for en google cloud
   ...
   for i in diml:
      ...
      random.seed(0)
      if dr!="RAW":
         dimReductionModels(X_dir="dataSets",dim= i,drmethod=dr,norm=isnorm)
      x_train = dataGen(DR=dr,data_file=ds[0],Lmin=26,Lmax=Lmax,istrain=False,norm=isnorm)
      x_valid = dataGen(DR=dr, data_file=ds[1],Lmin=26,Lmax=Lmax, istrain=False, norm=isnorm)
      x_test  = dataGen(DR=dr, data_file=ds[2],Lmin=26,Lmax=Lmax, istrain=False, norm=isnorm)
      ...
      N_valid = 5  #len(x_valid)  # number of validation  chain proteins
      N_test  = 5  #len(x_test)
      ...
      strore_folder=Store_model(k_model=model, Metric_res=results, fold_out=folder,
